...
// Record the latest request latency for each lyric source
const lyricSourceTimings: Record<string, number[]> = {};

// Periodically ping each lyric source API for latency
function startLyricSourcePingTimer() {
  async function pingSource(name: string): Promise<number> {
    const start = Date.now();
    try {
      if (name === "LrcLib") {
        await axios.get("https://lrclib.net/api/search", {
          params: { track_name: "ping", artist_name: "ping" },
          timeout: 5000,
        });
      } else if (name === "Netease") {
        await axios.get(
          "https://netease-cloud-music-api-psi-silk.vercel.app/search",
          { params: { keywords: "ping", type: 1 }, timeout: 5000 }
        );
      } else if (name === "QQMusic") {
        await axios.get("https://c.y.qq.com/soso/fcgi-bin/client_search_cp", {
          params: { w: "ping", format: "json", p: 1, n: 1 },
          timeout: 5000,
        });
      }
  ...
  async function pingAll() {
    const sources = Object.keys(lyricSourceMap);
    await Promise.all(
      sources.map(async (name) => {
        const ms = await pingSource(name);
        if (!lyricSourceTimings[name]) lyricSourceTimings[name] = [];
        lyricSourceTimings[name].push(ms);
        if (lyricSourceTimings[name].length > 10)
          lyricSourceTimings[name].shift();
      })
    );
  }
  pingAll(); 
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "nowPlayingLyrics.showPermissionHelp",
      () => {
  ...
  const debouncedUpdate = debounce(async () => {
    try {
      ...
      if (trackId !== lastTrackId) {
        ...
        try {
          ...
          if (cached && Date.now() - cached.timestamp < 24 * 60 * 60 * 1000) {
            ...
            statusBarItem.command = "nowPlayingLyrics.showPanel";
            updatePanel();
            updateLyricSourceTimingsTooltip();
            return;
          }

          console.log("Fetching fresh lyrics");
          const lyricsResult = await fetchSynchronizedLyrics(
            track.artist,
            track.title
          );
      ...
      const currentLine = findCurrentLyricLine(
        currentLyrics,
        track.position * 1000
      );
      if (currentLine) {
...
Click Allow when macOS asks for permission. You can manage these anytime in System Settings → Privacy & Security → Automation.`;
    ...
    vscode.window
      .showInformationMessage(message, { modal: true, detail }, "Get Started")
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "nowPlayingLyrics.selectLyricSourceOrder",
      async () => {
        ...
        const currentOrder: string[] = Array.isArray(inspected?.globalValue)
        ...
        for (let i = 0; i < picks.length; i++) {
          const placeHolder =
            i === 0
              ? `Current order: ${currentOrder.join(" > ")}\n Select #${
                  i + 1
                } lyric source (highest priority first)`
          ...
          if (!pick) {
            vscode.window.showInformationMessage(
              "Cancelled lyric source order selection."
            );
            return;
          }
          order.push(pick);
        ...
        await config.update(
          "nowPlayingLyrics.lyricSourceOrder",
          order,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async () => {
        const message =
          "Now Playing Lyrics needs permission to access your music apps.";
...
Would you like to open System Settings now?`;
        ...
        if (selection === "Open Settings") {
          ...
          await vscode.env.openExternal(
            vscode.Uri.parse(
              "https://github.com/maniyadv/vscode-now-playing-lyrics#permissions"
            )
          );
        }
      }
  ...
  // update tooltip for lyric source timings
  function updateLyricSourceTimingsTooltip() {
    const avgTimings = getLyricSourceAvgTimings();
    console.log("avgTimings:", avgTimings);
    const timingInfo = Object.entries(avgTimings)
      .map(([key, ms]) => `${key}: ${getTimingBadge(ms)}`)
      .join("\n");
    if (timingInfo) {
...
async function fetchSynchronizedLyrics(
  artist: string,
  title: string
): Promise<{
  syncedLyrics: LyricLine[];
  ...
  const config = vscode.workspace.getConfiguration(undefined, null);
  const inspected = config.inspect("nowPlayingLyrics.lyricSourceOrder");
  const order: string[] = Array.isArray(inspected?.globalValue)
    ? (inspected.globalValue as string[])
  ...
  for (const sourceName of order) {
    try {
      const result = await Promise.race([
        lyricSourceMap[sourceName as keyof typeof lyricSourceMap](
          artist,
          title
        ),
      ...
      if ((result as any).syncedLyrics?.length > 0) {
        return {
          ...(result as { syncedLyrics: LyricLine[]; plainLyrics: string }),
...
function getLyricSourceAvgTimings(): Record<string, number> {
  const avg: Record<string, number> = {};
  for (const key in lyricSourceTimings) {
    const arr = lyricSourceTimings[key];
    if (arr && arr.length > 0) {
      avg[key] = Math.round(arr.reduce((a, b) => a + b, 0) / arr.length);
    }
  }
  return avg;
}

// Generate colored badge
function getTimingBadge(ms: number): string {
...
async function fetchFromLrcLib(
  artist: string,
  title: string
): Promise<{ syncedLyrics: LyricLine[]; plainLyrics: string }> {
  ...
  for (const result of sortedResults) {
    try {
      const lyricsResponse = await axios.get(
        `https://lrclib.net/api/get/${result.id}`
      );
      ...
      if (lyricsData.syncedLyrics?.length > 0) {
        return {
          ...
          plainLyrics:
            ...
            lyricsData.syncedLyrics.map((l) => l.text).join("\n"),
        };
      }
    } catch (error) {
...
async function fetchFromNetease(
  ...
  try {
    ...
    // Then fetch lyrics
    const lyricsResponse = await axios.get(
      `https://netease-cloud-music-api-psi-silk.vercel.app/lyric?id=${songId}`
    );
    ...
    const lines = lyricsResponse.data.lrc.lyric
      ...
      .filter(
        (line: LyricLine | null): line is LyricLine =>
          line !== null && line.text !== ""
      );
...
async function fetchFromQQMusic(
  artist: string,
  title: string
): Promise<{ syncedLyrics: LyricLine[]; plainLyrics: string }> {
...
function sortSearchResults(
  ...
  return results.sort((a, b) => {
    ...
    const aArtistMatch =
      a.artistName.toLowerCase() === targetArtist.toLowerCase();
    const bArtistMatch =
      b.artistName.toLowerCase() === targetArtist.toLowerCase();
...
function findCurrentLyricLine(
  lyrics: LyricLine[],
  currentTimeMs: number
): LyricLine | null {
  return (
    lyrics.find((line, index) => {
      const nextLine = lyrics[index + 1];
      return (
        line.time <= currentTimeMs &&
        (!nextLine || nextLine.time > currentTimeMs)
      );
    }) || null
  );
}
